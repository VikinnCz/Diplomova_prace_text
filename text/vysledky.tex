...
%TODO: Blokové schéma rozdělení systému na jednotlivé části.
...
Jak je popsáno v bodě \ref{sec:Spectoda} systém Spectoda obsahuje základní animace, které jsou skládány do bloků a tím vznikají komplexní animace. Systém využívá již připravených bloků animací, které jsou uloženy jako objekty. Těmto objektům jsou přiřazeny parametry $code$, $characteristics$, $length$ a $colors$. Jejich struktura je zapsaná datovou třídou $AnimationBlock$. Z objektů tipu třídy jsou dále tvořeny datasety zapsané datovou třídou $Dataset$. Tyto instance obsahují 5 bloků animací a parametry $genre$, $tempo$ a $mood$. UML diagramy tříd jsou zobrazeny na obrázku \ref{fig:UML_diagram_Dataset_AnimationBlock}.
...
TODO: Popis charakteristiky bloků jejich statické hodnoty a vysvětlení co která hodnota znamená. 
...
\section{Realzace} \label{sec:Realizace}
